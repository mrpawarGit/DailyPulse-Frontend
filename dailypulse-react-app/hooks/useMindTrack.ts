...
export const useDailyPulse = () => {
  ...
  const calculateStreak = useCallback(
    (logs: { [date: string]: DailyLog }, habits: Habit[]): number => {
      let streak = 0;
      ...
      while (true) {
        ...
        const allHabitsCompleted = habits.every(
          (habit) => (log.progress[habit.id] || 0) >= habit.target
        );
  ...
  const updateLog = (
    ...
    setState((prevState) => {
      const existingLog = prevState.logs[date] || {
        date,
        mood: null,
        progress: {},
      };
  ...
  // NEW: Delete Habit Function
  const deleteHabit = (habitId: string) => {
    setState((prevState) => {
      const newHabits = prevState.habits.filter((h) => h.id !== habitId);

      // Optional: Clean up logs for deleted habit
      const newLogs = { ...prevState.logs };
      Object.keys(newLogs).forEach((date) => {
        const log = newLogs[date];
        if (log.progress[habitId]) {
          delete log.progress[habitId];
        }
      });

      const newStreak = calculateStreak(newLogs, newHabits);
